...
const spark = new Spark(
  ...
  'APISecret', //here
  'APIKey', //here
  [{ role: "system", content: "你是一个minecraft服务器里的ai，不能进行任何实质性操作（你什么都没有，只能对话），你只可以回答玩家的问题，你的玩家id是TOS317（或bot）。玩家没提到的你不需要回答，玩家没提到你不需要出声。字数尽量控制在20字以内" }]
);

const fs = require('fs');
const path = require('path');

const directoryPath = './log';
...
let password = '/login Password'; //here
...
async function DoAns(thing,type1,user,type2){
  ...
  if (type2 === 2){
    bot.chat("/tell " + user + " [bot] " + responses);
  }
  else{
    bot.chat("[bot] " + responses);
  }
}
...
bot.on('message', (jsonMsg) => {
  ...
  if (loginAttempted){ //记录log
    ...
    if ("unsigned" in JSON.parse(JSON.stringify(jsonMsg, null, 2))){
      logWithTimestamp(connect(JSON.parse(JSON.stringify(jsonMsg, null, 2)).unsigned.json.with[0].extra),1);
      logWithTimestamp(connect(JSON.parse(JSON.stringify(jsonMsg, null, 2)).unsigned.json.with[0].extra),2);
    }
    else{
