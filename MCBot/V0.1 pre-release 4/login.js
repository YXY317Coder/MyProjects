...
const plugin = JSON.parse(fs.readFileSync(path.join(__dirname,'./plugin/settings.json'),'utf-8'));
const plugins = [];
const events = [];

for (let i = 0;i < plugin.plugin.length;i++){
  plugins.push(require('./plugin/' + plugin.plugin[i] + '/' + plugin.plugin[i])); //后期用path.join替换
}

for (let i = 0;i < plugin.plugin.length;i++){
  let temp = JSON.parse(fs.readFileSync('./plugin/' + plugin.plugin[i] + '/settings.json','utf-8')); //后期用path.join替换
  for (let j = 0;j < temp.script.length;j++){
    events.push({'script':temp.script[j],'func': temp.func[j],'id':i});
  }
}

// events.sort((a,b) => a.script - b.script);
...
let loginAttempted = !settings.needlogin;
let loginTimer = null;
...
function logWithTimestamp(message,type) {
  ...
  // ✅ 自动创建文件（无需判断是否存在）
  if (settings.log){
    if (!(message.includes("&hide") && settings.canhide)){
...
bot.on('message', async (jsonMsg) => {
  ...
  if (loginAttempted && "unsigned" in JSON.parse(JSON.stringify(jsonMsg, null, 2))) {
    ...
    if (settings.checkplayer && settings.basic && (settings.checkplayerlist.includes(optext.match(/<(.*?)>/)[1]) || !settings.checkplayer)){
      if (optext.includes("&set history ")){ //以后用settings.txt替换
        ...
        if ((message.split("&talk ")[1] + " ").split(" ")[1] == ""){
          bot.chat("[bot admin " + optext.match(/<(.*?)>/)[1] + "] " + message.split("&talk ")[1].split(" ")[0]);
        }
        else{
          let temp = [];
          if (optext.split("&talk ")[1].split(" ")[1] == "any"){
            temp = Array.from(Object.keys(bot.players)).sort();
          }
          else{
            temp = JSON.parse(optext.split("&talk ")[1].split(" ")[1]);
          }
          for (let i = 0;i < temp.length;i++){
            if (temp[i] != 'TOS317'){
              bot.chat("/tell " + temp[i] + " [bot admin " + optext.match(/<(.*?)>/)[1] + "] " + optext.split("&talk ")[1].split(" ")[0]);
            }
            await sleep(1050);
          }
        }
      ...
      else if (optext.includes("&plugin")){
        bot.chat("[bot plugin] " + plugin.plugin);
        console.log(plugins,events)
      }
    ...
    for (let i = 0;i < events.length;i++){
      if (events[i].script === 'playerMessage'){
        // console.log(events[i].script);
        plugins[events[i].id](events[i].func,settings,bot,optext);
      }
    }
  }
  else{
    console.log(nowtime,message);
    if (settings.checkplayer && settings.basic && (settings.checkplayerlist.includes(message.split(" ")[0]) || !settings.checkplayer)){
      if (mode === 1){
        if (message.includes(" whispers ")){
    ...
    for (let i = 0;i < events.length;i++){
      if (events[i].script === 'playerMessage'){
        // console.log(events[i].script);
        plugins[events[i].id](events[i].func,settings,bot,message);
      }
    }
  }
  // 检查服务器消息中的登录提示
  ...
  if (serverLoginPrompts.some(prompt => lowerMessage.includes(prompt.toLowerCase()))) {
    ...
    if (!loginAttempted) {
      ...
      for (let i = 0;i < events.length;i++){
        if (events[i].script === 'login'){
          // console.log(events[i].script);
          plugins[events[i].id](events[i].func,settings,bot);
        }
      }
    }
  }
...
bot.on('kicked', async (reason) => {
  ...
  // }
  for (let i = 0;i < events.length;i++){
    if (events[i].script === 'kicked'){
      // console.log(events[i].script);
      plugins[events[i].id](events[i].func,settings,bot,reason);
    }
  }
  sleep(1500);
  process.exit(1);
...
bot.on('playerJoined', async (player) => {
  if (loginAttempted && player.username != 'TOS317'){
    ...
    for (let i = 0;i < events.length;i++){
      if (events[i].script === 'strictplayerJoined'){
        // console.log(events[i].script);
        plugins[events[i].id](events[i].func,bot,player);
      }
    }
  }
});
...
bot.on('error', async (err) => {
  ...
  for (let i = 0;i < events.length;i++){
    if (events[i].script === 'error'){
      // console.log(events[i].script);
      plugins[events[i].id](events[i].func,settings,bot,err);
    }
  }
  sleep(1500);
  process.exit(1);
...
// 以下是plugins处理部分
bot.on('message', async (jsonMsg) => {
  if (loginAttempted || !settings.needlogin){
    for (let i = 0;i < events.length;i++){
      if (events[i].script === 'message'){
        // console.log(events[i].script);
        plugins[events[i].id](events[i].func,settings,bot,jsonMsg);
      }
    }
  }
})
...
bot.on('playerJoined', async (player) => {
  if (loginAttempted || !settings.needlogin){
    for (let i = 0;i < events.length;i++){
      if (events[i].script === 'playerJoined'){
        // console.log(events[i].script);
        plugins[events[i].id](events[i].func,settings,bot,player);
      }
    }
  }
})
...
bot.on('playerUpdated', async (player) => {
  if (loginAttempted || !settings.needlogin){
    for (let i = 0;i < events.length;i++){
      if (events[i].script === 'playerUpdated'){
        // console.log(events[i].script);
        plugins[events[i].id](events[i].func,settings,bot,player);
      }
    }
...
bot.on('blockUpdate', async (oldBlock,newBlock) => {
  for (let i = 0;i < events.length;i++){
    if (events[i].script === 'blockUpdate'){
      // console.log(events[i].script);
      plugins[events[i].id](events[i].func,settings,bot,oldBlock,newBlock);
    }
  }
})

/*写plugins:
plugin/xxx/settings.json
func列表:函数名
script列表:触发器，要与func对应

plugin/xxx/xxx.js
*要打包为能用require的javascript程序，仅module.export = func
*require minflayer
*必备:do函数，参数:
func函数名,settings,bot,....args
